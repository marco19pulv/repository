...
=== SQL Views ===  
CREATE VIEW view_name AS SELECT column1, column2 FROM table_name WHERE condition;  # Crea una vista (query salvata)  
SELECT * FROM view_name;                        # Seleziona dalla vista  

=== SQL ALTER TABLE ===  
ALTER TABLE table_name ADD column_name datatype;   # Aggiunge una colonna alla tabella  
ALTER TABLE table_name DROP COLUMN column_name;    # Rimuove una colonna dalla tabella  
ALTER TABLE table_name MODIFY COLUMN column_name new_datatype;  # Modifica una colonna  

=== SQL Triggers ===  
CREATE TRIGGER trigger_name BEFORE|AFTER INSERT|UPDATE|DELETE ON table_name FOR EACH ROW BEGIN ... END;  # Crea un trigger  
DROP TRIGGER trigger_name;                         # Rimuove un trigger  
